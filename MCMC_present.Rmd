...
#pick a number out of distribution for theta star
theta_star <- rnorm(1,prior_mu,prior_sig)

#Joint with theta star as mean
theta_k <- prod(rnorm(15,prior_mu,sqrt(prior_sig)))*
  rnorm(1,)
...
#chain <- function(data,samples,mu_init,proposal_width,
 #                 prior_mu,prior_sd){
#  likelihood_sd <- sd(data)
#}
...
#Pick random starting mu 
mu_current <- rnorm(1, 70, 12)
proposal_width <- 3 # sigma square tune, can 'tune' 

#suggest a dumb Metropolis new position
mu_proposal <- rnorm(1,mu_current, sqrt(proposal_width)) 
y_proposal <- rnorm(100000, mu_proposal, sqrt(proposal_width))
pdf_proposal <- approxfun(y_proposal)(y_proposal) #linear interpolation of probabilities at data points

#Compute likelihood, multiply probabilities of each data point
# "usually you would use log probabilities but we omit this here"
likelihood_current <- rnorm(1,mu_current,sqrt(proposal_width))*pdf_proposal

plot(density(pdf_proposal),xlim=c(45,80))
lines(sort(y_proposal),col="red")

likelihood_proposal <- rnorm(1,mu_current,sqrt(proposal_width))*
  (approxfun(density())()) #linear interpolation of probabilities at data points

plot(density(likelihood_current),col="goldenrod")
```

```{r}
#Sampler 
sampler <- function(data, samples, mu_init=0.5, proposal_width=0.5,
                    prior_mu,prior_var, likelihood_var){
  x<-seq(-3,3,length.out=5000) #what are these from?
  posterior <- mu_init
  ...
  prior_current <- rnorm(1, prior_mu, sqrt(prior_sig))*dnorm(data)$y
  prior_proposal <- rnorm(1, prior_mu, sqrt(prior_sig))*dnorm(data)$y
