...
# mu ~ normal
#Sampler 
sampler <- function(data, samples, proposal_width, prior_mu,prior_sig2){
  chain <- c()
  like_data_pdf <- density(data)
  ...
  for(i in 2:samples){
    y_current <- rnorm(100000, mu_current, sqrt(proposal_width))
    ...
    likelihood_current<-rnorm(1,mu_current,sqrt(proposal_width))*
      approx(like_data_pdf,xout=mu_current)$y
    likelihood_proposal<-rnorm(1,mu_proposal,sqrt(proposal_width))*
      approx(like_data_pdf,xout=mu_proposal)$y

    #Prior
    prior_current <- rnorm(1, prior_mu, sqrt(prior_sig2))*
      approx(prior_data_pdf,xout=mu_current)$y
    prior_proposal <- rnorm(1, prior_mu, sqrt(prior_sig2))*
      approx(prior_data_pdf,xout=mu_proposal)$y
...
```{r}
test <- sampler(data=y,samples=30,proposal_width = 9,
                prior_mu=70,prior_sig2=12)
